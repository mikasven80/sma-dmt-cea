...
## treatment-durability cliff. Reproduces the Excel "OBM-Durability" analysis:
...
eval_oa_payment <- function(scheme, durability = Inf, faithful_excel = TRUE) {
  ov <- switch(scheme,
    Upfront  = NULL,
    ...
    stop("unknown scheme"))
  evaluate_arm("OA", faithful_excel = faithful_excel,
...
obm_durability_table <- function(endpoint = "VFS", durabilities = 1:9,
                                 faithful_excel = TRUE) {
  bsc <- evaluate_arm("BSC", faithful_excel = faithful_excel)
  ob5  <- paste0("OB5_",  endpoint); ob10 <- paste0("OB10_", endpoint)
  rows <- lapply(c(durabilities, Inf), function(d) {
    up  <- eval_oa_payment("Upfront",  d, faithful_excel)
    o5  <- eval_oa_payment(ob5,        d, faithful_excel)
    o10 <- eval_oa_payment(ob10,       d, faithful_excel)
